...
function hastag_gallery_meta_box() {
  add_meta_box(
   'ht gallery_Description',     // $id aa id che
   'Gallery Description',     // $title aya title
   'hastags_gallery_callback',      // $callback
   'hashtag_gb',                 // $page aa post type che
   'normal',                  // $context
   'high'                     // $priority
  );
}
...
// get karveno code
// gallery page image get
add_shortcode('gallery_page_image','gallery_page_image'); //[gallery_page_image]
function gallery_page_image(){
    $html = '';
    $args = array(
        'post_type'      => 'products',
        'posts_per_page' => '-1',
    ...
    if($query->have_posts()){
        $html .='<div class="gallery_img_box gallery_box_inner">';
            while($query->have_posts()){
                $query->the_post() ;
                global $post;
                $id = $post->ID;

                $gallery_img = get_post_meta( $id, 'gallery_img', true );
                $image_title = get_post_meta( $id, 'image_title', true );
                $explode = explode(",", $gallery_img);
                 // $gly_img = wp_get_attachment_url($explode[$i]);
                // pre($explode);
                $title = get_the_title();
                $titleTxt = "";
                // if ($title == "Roller Shutters") {

                //     $titleTxt ="Roller Shutter Motor - NICE brand";
                // }
                // else if($title == "Security Doors")
                // {
                //     $titleTxt = "Window Grill - Stainless Steel";
                // }
                // else{
                //     $titleTxt = $title;
                // }
                ...
                for ($i=0; $i < count($explode); $i++) { 
                    ...
                    if (@$gly_img) {
                        ...
                        $html .='<div class="iamage_box img_'.$count.'">';
                            $html .='<img src="'.$gly_img.'" class="img_gallery">';
                            $html .='<div class="hover_title_image">';
                                $html .='<div class="hover_inner_title">';
                                    $html .='<span>'.$image_title[$i].'</span>';
                                $html .='</div>';
                            $html .='</div>';
                        $html .='</div>';
                    }
